...
//****************************************************************************************************************************************************************
//setpoint
double OGN_setpoint = 0;  // Góc hiện tại và góc mong muốn
...
// Biến tính toán
SimpleKalmanFilter kalmanFilter(2, 2, 0.036); // Khởi tạo Kalman Filter (measurement noise, process noise, dt)
...
// Fuzzy membership functions (example)
float fuzzyKp(float Error) {
    if (Error > 10) return 800; // High error
    else if (Error > 5) return 600; // Medium error
    else return 400; // Low error
}

float fuzzyKi(float Error) {
    if (Error > 10) return 0; // No integral action
    else if (Error > 5) return 0.1; // Small integral action
    else return 0.5; // Larger integral action
}

float fuzzyKd(float deltaError) {
    if (deltaError > 5) return 10; // High change in error
    else if (deltaError > 2) return 5; // Medium change in error
    else return 1; // Low change in error
}
...
void readMPU6050(sensors_event_t &a, sensors_event_t &g, sensors_event_t &temp) 
{
    ...
    float angle = 0.2 * (lastAngle + gyroRate * 0.036) + 0.8 * angleAcc;
    lastAngle = angle;

    
    // Lọc góc bằng Kalman Filter
    angleFiltered = kalmanFilter.updateEstimate(angle);  // Sử dụng phương thức updateEstimate() của Kalman Filter
...
void computePID() {
    ...
    // Adjust PID parameters using fuzzy logic
    Kp = fuzzyKp(Error);
    Ki = fuzzyKi(Error);
    Kd = fuzzyKd(deltaError);
