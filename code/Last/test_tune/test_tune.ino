...
// Thông tin Wi-Fi
const char* ssid = "AnTran"; 
const char* password = "16052003."; 

// Thông tin MQTT
const char* mqttServer = "192.168.8.117"; // Địa chỉ broker MQTT
...
double Kp = 60 , Ki = 30 , Kd = 20;
double Kp_node, Ki_node, Kd_node;
double Kp_tune, Ki_tune, Kd_tune;
double input, output;
// Tạo đối tượng PID
...
// Biến tính toán
SimpleKalmanFilter kalmanFilter(1, 0.5, 0.052); // Khởi tạo Kalman Filter (measurement noise, process noise, dt)
...
float speedRPM; // Tốc độ động cơ (RPM)
float speedAngular; // Tốc độ góc (rad/s)
float speedLinear; // Tốc độ tịnh tiến (m/s)
const int pulsesPerRevolution = 330; // Số xung mỗi vòng quay
const float wheelDiameter = 0.065; // Đường kính bánh xe (m) //https://hshop.vn/bo-dong-co-dc-servo-jgb37-520-kem-ga-bat-va-banh-xe-v2-65mm
...
void reconnectMQTT() 
  {
    // Nếu không có kết nối, thực hiện kết nối lại
    while (!client.connected()) 
      {
        Serial.print("Đang kết nối tới MQTT...");
        
        // Cố gắng kết nối đến broker MQTT
        if (client.connect("ESP32Client")) 
          {
            Serial.println("Đã kết nối!");
            client.subscribe("esp32/Kp_tune");
            client.subscribe("esp32/Ki_tune");
            client.subscribe("esp32/Kd_tune");
          } 
        else 
          {
            Serial.print("Lỗi kết nối, cố gắng lại sau 5 giây...");
            delay(5000);
          }
      }
  }
...
void sendMQTTData() 
  {
    if (!client.connected()) 
      {
        reconnectMQTT(); // Kiểm tra và kết nối lại nếu MQTT bị mất kết nối
      }
  
    // Gửi từng giá trị vào các topic khác nhau
    client.publish("esp32/input", String(input).c_str()); 
    client.publish("esp32/E", String(Error).c_str());
    client.publish("esp32/output", String(output).c_str());
    client.publish("esp32/Kp_node", String(Kp_node).c_str()); 
    client.publish("esp32/Ki_node", String(Ki_node).c_str());
    client.publish("esp32/Kd_node", String(Kd_node).c_str());
    client.publish("esp32/RPM", String(abs(speedRPM)).c_str());
  }
...
void callback(char* topic, byte* payload, unsigned int length) 
    {   
        // Xử lý theo topic
        if (strcmp(topic, "esp32/Kp_tune") == 0) 
          {    
            Kp += 0.5; // Tăng Kp lên 0.5           
          } 
        else if (strcmp(topic, "esp32/Ki_tune") == 0) 
          {
            Ki += 0.1; // Tăng Kp lên 0.5  
          } 
        else if (strcmp(topic, "esp32/Kd_tune") == 0) 
          {
            Kd += 0.1;
          }
    }
...
void setup() 
    ...
    client.setCallback(callback); // Đăng ký hàm callback để xử lý dữ liệu MQTT
      ...
      //ENCODER
      ESP32Encoder::useInternalWeakPullResistors = puType::up;      // Bật điện trở kéo lên
      encoder.attachHalfQuad(26, 27);        // Kết nối encoder với các chân A và B
      encoder.clearCount();         // Xóa giá trị cũ và khởi tạo bộ đếm
...
void calculateSpeed() 
    {
        ...
        Serial.print("\nTốc độ (RPM): ");
        Serial.print(abs(speedRPM));
        Serial.print(" RPM\n");
    
        Serial.print("Tốc độ góc: ");
        Serial.print(abs(speedAngular));
        Serial.print(" rad/s\n");
    
        Serial.print("Tốc độ xe: ");
        Serial.print(abs(speedLinear));
        Serial.println(" m/s");
...
void loop() 
  {
    ...
    dt = (currentMillis - lastTime); // Tính toán thời gian trôi qua (ms)
    ...
    client.loop(); // Cập nhật trạng thái MQTT
    ...
    sendMQTTData();    // Gửi dữ liệu lên Node-RED qua MQTT
  }
